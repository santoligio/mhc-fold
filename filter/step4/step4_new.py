...
def process_structure(
    ...
    for key in dssp.keys():
        chain_id_dssp, res_id = key
        ss = dssp[key][2]

        if ss in ("H", "G", "I"):
            helix_residues.add((chain_id_dssp, res_id[1]))

    # --- Collect helix atoms ---
    helix_atoms = []
    for chain_id_dssp, resseq in helix_residues:
        if chain_id_dssp in model:
            chain_obj = model[chain_id_dssp]
            if resseq in chain_obj:
                for atom in chain_obj[resseq]:
    ...
    for atom in helix_atoms:
        ...
        for neighbor in neighbors:
            ...
            # keep only hetero / non-standard residues
            if residue.id[0].strip() != "":
                chain_id_n = residue.get_parent().id
                resseq_n = residue.id[1]
                extra_residues.add((chain_id_n, resseq_n))
